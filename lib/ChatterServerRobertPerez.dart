// ChatterServerRobertPerez.dart.  This is a GUI demo of socket connections.
// Robert Perez
// working from notes from Suragch

// This runs with ChatterClientRobertPerez.dart.  Run this ChatterServerRobertPerez.dart first, then
// run ChatterClientRobertPerez.dart along with it.  They should communicate.
/*
   must have
...
class SaidState
{
   ...
   SaidState( this.conversation );
}

class SaidCubit extends Cubit<SaidState>
{
  SaidCubit() : super( SaidState([]) );

  // void update( String more ) { emit(SaidState( "${state.said}$more\n" ) ); }
  void update( String s ) {
    List<String> updatedConv = state.conversation;
    updatedConv.add(s);
    emit( SaidState(updatedConv) );
  }
}
    ...
    ( appBar: AppBar( title: Text("server") ),
      ...
      ( children:
        [ // place to type message
          ...
          cs.listening
          ? cs.theClient!=null
            ? Column(children: [for(String s in sc.state.conversation) Text(s)])
